...
## BUG-018: 点状小图层处理后地图没有可见结果

- 状态：已修复
- 日期：2026-05-29
- 场景：选择 `gis_osm_traffic_free_1` 等点状或小要素图层，仅导出该图层后查看地图结果。
- 现象：结果面板显示已有输出要素数量，但地图上只看到底图，处理结果不明显或不可见。
- 影响：用户会误以为小图层没有被处理或没有返回数据。
- 涉及文件：`src/components/MapPreview.tsx`。
- 尝试：检查结果面板要素数量与地图样式配置，确认当前 Vector / WebGL 样式只覆盖面填充和线描边，没有给点要素配置符号。
- 解决方式：为结果图层、轻量预览图层和原始对比图层同时补充点符号样式；WebGL 模式增加 `circle-*` 样式，Canvas 模式增加 `CircleStyle`。
- 验证：
  - `npm run typecheck` — 通过。
  - `npm run build` — 通过。
- 后续步骤：用点状 OSM 图层手动复测地图可见性。

---

## BUG-019: tauri:dev 被旧 Vite 进程占用后加载错端口

- 状态：已修复
- 日期：2026-05-30
- 场景：运行 `npm run tauri:dev` 时，旧的 `vite --host 127.0.0.1` 进程占用 5173。
- 现象：新的 Vite 自动切到 5174，但 `src-tauri/tauri.conf.json` 的 `build.devUrl` 固定为 `http://127.0.0.1:5173`，Tauri 桌面端可能加载旧服务或空页面。
- 影响：用户会误以为桌面端大模型不可用，实际是 Tauri 和当前 Vite dev server 端口不一致。
- 涉及文件：`src-tauri/tauri.conf.json`, `package.json`。
- 尝试：检查 `tauri:dev` 日志、5173 端口占用进程和 Tauri 桌面进程；确认占用者是同项目旧 Vite 进程。
- 解决方式：停止旧 Vite 进程和旧 Tauri 桌面进程，重新运行 `npm run tauri:dev`，Vite 回到 5173，Tauri devUrl 与前端服务一致。
- 验证：
  - `npm run tauri:dev` — Vite 监听 `http://127.0.0.1:5173/`，Tauri exe 正常运行。
  - 使用 `TAURI_LLM_*` 配置直接请求 SiliconFlow `/v1/chat/completions` — 返回 `{"ok":true}`。
  - 浏览器自测上传 `test-data/河北省.geojson`，生成 AST 请求返回 200，确认执行生成 `河北省.geosurgical.geojson`。
- 后续步骤：如经常遇到端口占用，可考虑把 `dev:tauri` 改为固定端口并在占用时直接失败，而不是自动切换端口。
