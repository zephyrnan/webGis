...
- Attempts: Ran `npm audit --registry https://registry.npmjs.org --audit-level=high` against official npm registry.
- Resolution: Fixed；使用官方 npm registry 重跑 audit，结果为 0 vulnerabilities。
- Next Steps: None.
...
- Attempts: Removed `mut` from `null_key` and `f` in dispatcher.rs; removed dead `total_field_count` field from `DbfMetadata` struct in metadata.rs.
- Resolution: Rust warnings fixed (0 warnings from `cargo check`). Vite chunk-size warning remains, planned for P0.4.
- Next Steps: P0.4 dynamic import for OpenLayers/WASM to reduce bundle size.
...
- Status: Fixed
- Date: 2026-05-24
- Scenario: Upload GeoJSON with an area=0 feature, click shortcut tag "清理 area 为 0 的废弃多边形", generate AST.
- Symptom: Mock Brain generates `{ "action": "filter_area", "field": "area", "operator": ">=", "value": 0 }`. The `>=` operator keeps all features (including area=0), so nothing is removed.
- Impact: The "删除 area 为 0 的要素" shortcut tag doesn't actually remove zero-area features in Mock Brain mode.
- Files: `src/services/brain.ts`, `src/services/brain.test.ts`.
- Attempts: Added `mentionsEqualToZero()` function to detect "为 0", "等于 0", "is 0", "equals 0", "= 0" patterns. Updated operator selection logic to use `>` when value is 0 and command mentions equality to zero.
- Resolution: Fixed；新增 2 个测试用例覆盖中英文场景，全部 9 个测试通过。
- Next Steps: None.
...
- Status: Fixed
- Date: 2026-05-24
- Scenario: Execute any AST operation, switch UI language to English, check the operation log in ResultPanel.
- Symptom: Log shows "Mock WASM 已执行 filter_area (移除了 0 个要素)" in Chinese even when UI is in English.
- Impact: Operation log messages are not i18n'd, breaking the 6-language experience.
- Files: `src-wasm/src/dispatcher.rs`, `src/components/ResultPanel.tsx`, `src/i18n/locales.ts`.
- Attempts: Observed during manual browser verification.
- Resolution: Fixed.
...
- Status: Fixed
- Date: 2026-05-25
- Scenario: Comprehensive code review identified multiple MEDIUM severity issues across Rust WASM, Worker, and frontend components.
- Symptom: Various logic errors, memory leaks, and missing error handling.
- Impact: Individual issues are medium-severity but collectively affect reliability and memory efficiency.
- Files: `src-wasm/src/dispatcher.rs`, `src/components/MapPreview.tsx`, `src/components/HistoryPanel.tsx`, `src/workers/geosurgical.worker.ts`, `src/hooks/useGeoSurgicalWorker.ts`.
...
9. **Hardcoded Chinese in `emit_progress`** — Fixed remaining hardcoded Chinese strings in Rust `emit_progress` calls (main dispatcher loop, stream_export_zip messages).

- Resolution: `cargo check` clean, `tsc --noEmit` clean, all 45 tests pass.
- Next Steps: H3 (Clip ≠ Intersect) requires Sutherland-Hodgman algorithm implementation — deferred.
